...
const COLLECTIVES_ROCOCO_ED: CollectivesBalance =
	collectives_westend_runtime::ExistentialDeposit::get();
...
pub fn collectives_rococo_session_keys(keys: AuraId) -> collectives_westend_runtime::SessionKeys {
    collectives_westend_runtime::SessionKeys { aura: keys }
}
...
pub fn collectives_rococo_local_config() -> GenericChainSpec {
    let mut properties = sc_chain_spec::Properties::new();
    properties.insert("ss58Format".into(), 42.into());
    ...
    GenericChainSpec::builder(
        collectives_westend_runtime::WASM_BINARY
            .expect("WASM binary was not built, please build it!"),
        Extensions { relay_chain: "rococo-local".into(), para_id: 1001 },
    )
    .with_name("Rococo Collectives Local")
    .with_id("collectives_rococo_local")
    .with_chain_type(ChainType::Local)
    .with_genesis_config_patch(collectives_rococo_genesis(
        // initial collators.
        vec![
            (
                get_account_id_from_seed::<sr25519::Public>("Alice"),
    ...
    serde_json::json!( {
        "balances": {
            "balances": endowed_accounts
                .iter()
                .cloned()
                .map(|k| (k, COLLECTIVES_ROCOCO_ED * 4096))
                .collect::<Vec<_>>(),
        },
        "parachainInfo": {
            "parachainId": id,
        ...
        "collatorSelection": {
            "invulnerables": invulnerables.iter().cloned().map(|(acc, _)| acc).collect::<Vec<_>>(),
            "candidacyBond": COLLECTIVES_ROCOCO_ED * 16,
        },
        "session": {
            "keys": invulnerables
                .into_iter()
                .map(|(acc, aura)| {
                    (
                        acc.clone(),                             // account id
                        acc,                                     // validator id
                        collectives_rococo_session_keys(aura), // session keys
                    )
                })
                .collect::<Vec<_>>(),
        },
